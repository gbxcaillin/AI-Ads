---
name: ai-commercial
description: End-to-end recipe for a short brand commercial (30 to 45 seconds) from AI-generated clips, real screen captures of the product, one narration take and one music bed, stitched with ffmpeg into a reviewable cut. Use it whenever the user asks for a commercial, advert, promo, brand film, explainer, TV spot, social video ad, sizzle reel or a video for the website from OpenArt, Seedance, Veo, Kling or any text-to-video model, even for just the prompts, one clip, the voice-over, or stitching clips they already have. Use it for the failure modes it fixes, such as gibberish on-screen text, a lip-synced narrator, a wrong logo, jarring cuts, music that jumps or drops out, rushed or slow pacing. Use it too for any 9:16, vertical, portrait, Reels, Shorts, TikTok or Stories version of a spot, and when planning 16:9 clips that must later crop to 9:16. It composes the widescreen shot so the social cut is a crop, not a re-shoot, and bundles the reframe script and portrait recorders.
---
...
Bundled in `scripts/`: `stitch-commercial.py`, `narration-lines.py`, `record-tools-clip.mjs`, `record-endcard-clip.mjs`, their portrait siblings `record-tools-clip-9x16.mjs` and `record-endcard-clip-9x16.mjs`, `reframe-9x16.py` (turns a finished 16:9 cut into the 9:16 social version), `stitch-social-9x16.py` (cuts a 15 to 20 s native 9:16 piece: two or three portrait clips, one narration line from the approved take, the end card with its line, the bed trimmed to length), `captions-9x16.py` (burns phone-readable captions from the script and a transcript; needs `pip install faster-whisper` unless a words file is given, and `fonts/Montserrat-600.ttf`), and `fonts/site-embedded.css` (the site's web fonts as data URIs, so headless Chromium renders real type; rebuild this file for a different site).
...
5. **Burn in captions.** Social autoplays muted, so the narration has to be readable or the ad is a silent film. `scripts/captions-9x16.py <cut.mp4> <captions.json> <out.mp4>` takes the script lines as the words (a transcription slip never reaches the screen) and a word-level transcript as the timing (it transcribes the cut itself unless the spec names a words file), makes one caption per sentence, splitting anything over two lines at a comma, and holds each caption until the next so nothing flickers between phrases. Style: the site's Montserrat SemiBold at 76 px (about 4 percent of the height, the size platform captions use), off-white on a charcoal box, text bottom at 70 percent of the frame with 70 px side margins, which keeps it clear of the platform's own overlays and reads over a dark office and a white page alike. Set `stop_at` to the moment the end card's own text fades in; from there the card is the caption. Tile frames at caption moments and read them at phone size before sending. The GBX spec is `spots/gbx-commercial/captions-9x16.json`.

Clips cropped from 16:9 keep the master's resolution, so 720p sources look soft at 1080x1920. Reframe the 1080p master for the final, and regenerate at 9:16 only the scenes that the crop cannot hold.
